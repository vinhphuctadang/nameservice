...
import { Name } from "./module/types/nameservice/name";
...
export default {
    ...
    actions: {
        init({ dispatch, rootGetters }) {
            console.log('Vuex module: vinhphuctadang.nameservice.nameservice initialized!');
        ...
        async QueryInfo({ commit, rootGetters, getters }, { options: { subscribe, all } = { subscribe: false, all: false }, params, query = null }) {
            try {
                const key = params ?? {};
                const queryClient = await initQueryClient(rootGetters);
                let value = (await queryClient.queryInfo(key.name)).data;
                commit('QUERY', { query: 'Info', key: { params: { ...key }, query }, value });
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QueryInfo', payload: { options: { all }, params: { ...key }, query } });
                return getters['getInfo']({ params: { ...key }, query }) ?? {};
            }
            catch (e) {
                throw new SpVuexError('QueryClient:QueryInfo', 'API Node Unavailable. Could not perform query: ' + e.message);
            }
        },
        async sendMsgChangeSaleStatus({ rootGetters }, { value, fee = [], memo = '' }) {
            try {
                const txClient = await initTxClient(rootGetters);
                const msg = await txClient.msgChangeSaleStatus(value);
                ...
                if (e == MissingWalletError) {
                    throw new SpVuexError('TxClient:MsgChangeSaleStatus:Init', 'Could not initialize signing client. Wallet is required.');
                }
                else {
                    throw new SpVuexError('TxClient:MsgChangeSaleStatus:Send', 'Could not broadcast Tx: ' + e.message);
                }
            }
        },
        ...
        async MsgChangeSaleStatus({ rootGetters }, { value }) {
            try {
                const txClient = await initTxClient(rootGetters);
                const msg = await txClient.msgChangeSaleStatus(value);
                return msg;
                ...
                if (e == MissingWalletError) {
                    ...
                    throw new SpVuexError('TxClient:MsgChangeSaleStatus:Create', 'Could not create message: ' + e.message);
                }
            }
        },
