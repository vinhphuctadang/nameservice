...
const baseQueryInfoRequest: object = { name: "" };

export const QueryInfoRequest = {
  encode(message: QueryInfoRequest, writer: Writer = Writer.create()): Writer {
    if (message.name !== "") {
  ...
  decode(input: Reader | Uint8Array, length?: number): QueryInfoRequest {
    const reader = input instanceof Uint8Array ? new Reader(input) : input;
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = { ...baseQueryInfoRequest } as QueryInfoRequest;
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1:
...
const baseQueryInfoResponse: object = { owner: "", price: "", forSale: "" };

export const QueryInfoResponse = {
  encode(message: QueryInfoResponse, writer: Writer = Writer.create()): Writer {
    if (message.owner !== "") {
  ...
  decode(input: Reader | Uint8Array, length?: number): QueryInfoResponse {
    const reader = input instanceof Uint8Array ? new Reader(input) : input;
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = { ...baseQueryInfoResponse } as QueryInfoResponse;
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1:
...
export class QueryClientImpl implements Query {
  ...
  Params(request: QueryParamsRequest): Promise<QueryParamsResponse> {
    ...
    const promise = this.rpc.request(
      "vinhphuctadang.nameservice.nameservice.Query",
      "Params",
      data
    );
